...
def EV_F(X, k_init, n_agents):
    ...
    v = np.zeros(5)
    for i in range(5):
        v[i] = (1./5)*beta*V_INFINITY(theta[i], knext)
    VT_sum=utility(cons, lab) + v.sum()

    return VT_sum

# V infinity
def V_INFINITY(theta, k=[]):
    e=np.ones(len(k))
...
def EV_F_ITER(X, k_init, n_agents, grid):
    ...
    for i in range(5):
        VT_sum+= beta*grid[i].evaluate(knext)*(1./5)
...
def EV_G(X, k_init, n_agents, itheta):
    N=len(X)
    ...
    f_prod=output_f(theta[itheta], k_init, lab)
...
def EV_G_ITER(X, k_init, n_agents, itheta):
    N=len(X)
    M=3*n_agents+1  # number of constraints
    G=np.empty(M, float)
...
def EV_JAC_G(X, flag, k_init, n_agents, itheta):
    N=len(X)
    M=3*n_agents+1
    NZ=M*N
    A=np.empty(NZ, float)
    ...
    if (flag):
        ...
        # Finite Differences
        h=1e-4
        gx1=EV_G(X, k_init, n_agents, itheta)

        for ixM in range(M):
            for ixN in range(N):
                xAdj=np.copy(X)
                xAdj[ixN]=xAdj[ixN]+h
                gx2=EV_G(xAdj, k_init, n_agents, itheta)
...
def EV_JAC_G_ITER(X, flag, k_init, n_agents, itheta):
    N=len(X)
    M=3*n_agents+1
    NZ=M*N
    A=np.empty(NZ, float)
    ...
    if (flag):
        ...
        # Finite Differences
        h=1e-4
        gx1=EV_G_ITER(X, k_init, n_agents, itheta)

        for ixM in range(M):
            for ixN in range(N):
                xAdj=np.copy(X)
                xAdj[ixN]=xAdj[ixN]+h
                gx2=EV_G_ITER(xAdj, k_init, n_agents, itheta)
                A[ixN + ixM*N]=(gx2[ixM] - gx1[ixM])/h
        return A
